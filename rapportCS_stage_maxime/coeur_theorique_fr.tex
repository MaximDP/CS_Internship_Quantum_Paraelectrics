...
repartir des paramètres publiés, vérifier les facteurs de volume et reconstruire
le noyau envoyé au solveur. Chaque transformation suit donc le même schéma :

\begin{enumerate}[label=\textbf{Étape \arabic*.},leftmargin=5.2em,itemsep=2pt]
  \item préciser les variables, les unités et la convention de Fourier ;
  \item écrire le terme discret avant toute approximation ;
  \item effectuer analytiquement l'élimination ou le changement de variable ;
  \item identifier la quantité effectivement calculée par le code ;
\end{enumerate}
...
périodique d'une unité élémentaire ; il n'est donc pas surprenent qu'une description simple équivalente existe en
...
reprise explicitement dans la
\hyperref[sec:core_dipolar_kernel]{\textcolor{blue}{discussion du noyau
dipolaire (section~\ref*{sec:core_dipolar_kernel})}}.
...
Par élimination, il faut convenir qu'il ne s'agit que d'une élimination au sens où
la variable éliminée n'apparaît plus dans l'expression finale, son effet, lui, est toujours
pris en compte car on s'appuie essentiellement sur une identité algébrique exacte.
...
\end{equation}
Prolonger Eq.~\eqref{eq:core_sr_small_k} jusqu'au bord de zone rend
artificiellement instable le point $R$ pour certains jeux de paramètres. Le
solveur emploie donc Eq.~\eqref{eq:core_full_sr_kernel} dans toutes les
...
Eq.~\eqref{eq:core_dipole_surface_term}.
...
un champ intégré sur toutes ses trajectoires : éliminer la déformation
inhomogène avant cette étape était impossible, sans performer l'intégrale gaussienne.
...
dans la fermeture auto-cohérente. L'action polaire obtenue après cette étape,
et non une énergie de Landau ajustée, constitue le point d'entrée de la SCHA.
...
de basse énergie associé à l'Eq.~\eqref{eq:core_scha_full_bare_kernel} est
alors précisément la base cubique de l'Eq.~\eqref{eq:core_cubic_rank_two_basis} :
\begin{equation}
\begin{aligned}
...
l'Eq.~\eqref{eq:core_matsubara_transform} et le poids $e^{-\mathcal S_0/\hbar}$,
\begin{equation}
\mathcal S_0
=\frac12
...
introduite avant l'Eq.~\eqref{eq:core_acoustic_action}, en renommant sa
...
Eq.~\eqref{eq:core_variational_free_energy_compact}. En particulier,
\begin{align}
...
l'Eq.~\eqref{eq:core_inhomogeneous_self_energy} se déduit alors directement :
\begin{equation}
...
mineures de l'Eq.~\eqref{eq:core_inhomogeneous_vertex_parity} :
\begin{equation}
\mathcal M^{\rm inh}_{\alpha\mu,\beta\nu}(Q-K)
=\mathcal M^{\rm inh}_{\alpha\mu,\beta\nu}(K-Q)
...
S_K^{\mu\nu\mid ab}$. Par exemple, l'Eq.~\eqref{eq:core_f4loc_variation}
correspond à $\Pi^{\rm loc}_{\mu\nu}=3\beta_{\mu\nu\gamma\delta}
G^{\gamma\delta}$. En ajoutant la variation de $f_A+f_2$, obtenue dans
l'Eq.~\eqref{eq:core_fAf2_variation}, la stationnarité prend donc la forme
factorisée
\begin{equation}
...
définition de l'Eq.~\eqref{eq:core_symmetric_propagator_derivative} donne
\begin{equation}
A_{ra}(K)A_{sb}(K)\,
\mathcal S_K^{\mu\nu\mid ab}
=\delta_r^{(\mu}\delta_s^{\nu)}.
...
Eq.~\eqref{eq:core_equal_point_covariance} condense donc les intégrales
...
L'Eq.~\eqref{eq:core_full_scha_tensor} est l'équation tensorielle dont part le
calcul. La fermeture effectivement résolue adopte l'ansatz à masse isotrope
comme annoncé à l'Eq.~\eqref{eq:core_full_lattice_trial_kernel} :
\begin{equation}
...
La projection isotrope de l'Eq.~\eqref{eq:core_full_scha_tensor} réduit alors
...
l'Eq.~\eqref{eq:core_inhomogeneous_self_energy}. Contrairement aux autres
...
puis annule le résidu de l'Eq.~\eqref{eq:core_scalar_scha_closure} par
...
projetée issue de l'équation SCHA tensorielle ; son petit résidu ne démontre
pas la stationnarité de la fonctionnelle variationnelle complète. Cette forme montre explicitement les dépendances respectivement
...
\end{equation}
La courbure de l'énergie libre variationnelle après réoptimisation du noyau
n'est pas calculée ici ; l'identification à la réponse thermodynamique
complète demanderait ce contrôle supplémentaire.
Pour une mesure
...
\end{equation}
insérée dans l'Eq.~\eqref{eq:core_piezo_stationary_strain} donne alors la
formule suivante.
...
l'Eq.~\eqref{eq:core_physical_covariance}. Les équations non linéaires sont
...
\begin{enumerate}[label=\textbf{N\arabic*.},leftmargin=3.2em,itemsep=3pt]
  ...
  \item calculer $G^{(u)}$ par Eq.~\eqref{eq:core_physical_covariance}, avec
...
\end{enumerate}
Cette chaîne ne contient aucune trajectoire à échantillonner. Les seuls objets
discrétisés sont la zone de Brillouin et, lorsque la forme analytique n'est pas
disponible, une quadrature déterministe.
...
Contrôle & Critère \\
\hline
Convergence de la racine & Résidu du noyau sous le seuil fixé après résolution de Brent \\
...
$0{,}36$ à $0{,}53\,\%$ au plus. Cette incertitude est inférieure aux écarts de
modèle et aux incertitudes expérimentales ; une augmentation systématique de la
grille ne serait donc pas le premier levier d'amélioration.
...
contributions omises sur la réponse uniforme. Cette seconde calibration n'est
donc ni une pression extérieure prédite par la DFT ni l'identification d'un
mécanisme microscopique unique. La comparaison sépare ainsi la réponse sans
...
La loi de Barrett fournit l'interpolation quantique
\begin{equation}
\varepsilon_r(T)=
\frac{C}{\frac{T_1}{2}\coth\!\left(\frac{T_1}{2T}\right)-T_0}.
\label{eq:core_barrett_law}
\end{equation}
Pour BaTiO$_3$, les nombres utilisés comme référence ne sont cependant pas des
prédictions du modèle : Barrett rapporte $C=1.5\times10^5\,\mathrm K$ et
$T_0=390\,\mathrm K$ à partir de l'expérience~\cite{Barrett1952}. Dans la
région classique,
\begin{equation}
...
=\frac{\varepsilon_r^{\rm CW,exp}-6}{4\pi}.
...
\end{equation}
À chaque
température, Eq.~\eqref{eq:core_lattice_reconstruction} est inversée ; les
...
Sur la grille $40^3$, cette loi reproduit la maille cible avec une erreur
maximale de $1.92\times10^{-6}\,\mathring{\rm A}$ entre 400 et
$565\,\mathrm K$.
...
maille reproduit la cible structurale avec une erreur maximale de
...
pas une validation indépendante du Hamiltonien. Cette calibration n'est cependant pas sans intérêt d'un point de vue
informatif mais aussi prédictif : s'il s'avérait qu'elle peut être calibrée une fois pour toute, tout en permettant
de faire des prédictions futures par exemple.
...
Eq.~\eqref{eq:core_scalar_residual} et on résout le résidu en température le
...
Hamiltonien effectif réduit. La SCHA du présent travail reprend ce jeu
anharmonique, avec un choix explicite d’ancrage harmonique décrit ci-dessous. Il faut donc distinguer « même modèle réduit » de
...
Pour la reconstruction numérique, la courbure uniforme est fixée à
$A_{0,1}^{(u)}=2\kappa=-0.0404430732\,\mathrm{Ha/bohr^2}$.
La combinaison directe de $\kappa_2$, $j_i$, $Z^*$, $\varepsilon_\infty$ et
$a_0$ publiés donne plutôt
$\Phi_{\Gamma}^{\rm brut}=-0.0412428397\,\mathrm{Ha/bohr^2}$ dans la
convention d'Ewald retenue. Le code utilise donc
$\Phi(\mathbf k)-\Phi_{\Gamma}^{\rm brut}\mathbb I+A_1^{(u)}\mathbb I$,
soit un ancrage harmonique distinct, de décalage nu
$7.998\times10^{-4}\,\mathrm{Ha/bohr^2}$. Cet écart ne s'explique pas par le
seul arrondi des paramètres affichés et ne doit pas être confondu avec une
correction SCHA. La comparaison avec la MD publiée porte ainsi sur le même
potentiel anharmonique réduit, avec cette réserve harmonique et avec la
distinction entre statistiques classique et quantique. À $500\,\mathrm K$, pression nulle
et grille $40^3$, remplacer cet ancrage par la valeur brute augmente la
susceptibilité gaussienne d'environ $1.70\,\%$ ; ce test ponctuel ne constitue
pas une borne uniforme en température.
Les courbes conservent également la masse acoustique de calcul
$46.44\,\mathrm{amu}$, distincte des $46.64\,\mathrm{amu}$ de la
Table I de Nishimatsu~\cite{Nishimatsu2016}. À $500\,\mathrm K$, pression nulle
et grille $40^3$, employer cette dernière valeur modifie $\chi$ de moins de
$0.0003\,\%$ pour les jeux BaTiO$_3$ Nishimatsu et Mayer. La valeur historique
est indiquée pour permettre la reproduction exacte des courbes.
...
\end{tikzpicture}
\caption*{\small\textit{Schéma --- Réduction adiabatique employée par Mayer \emph{et al.}. Les modes
...
ensuite repris dans la SCHA. La comparaison avec la MD teste la fermeture
statistique, sous réserve des conventions harmoniques et dynamiques précisées
dans le texte. Schéma de l'auteur, d'après la procédure de la
réf.~\cite{Mayer2022}.}}
\end{figure}
...
Elle est ici une référence de \emph{benchmark}, et non une nouvelle mesure
expérimentale.
...
mesure directe. Adapté de la Fig.~3(a) de la réf.~\cite{Mayer2022}.}
...
Le résultat est instructif à deux niveaux. D'une part, l'amélioration obtenue
...
part, avec ce même potentiel enrichi, la fermeture SCHA centrée reste nettement
plus rigide que la réponse de la dynamique moléculaire et que sa tendance
expérimentale. Le désaccord ne peut donc pas être attribué au seul volume ni,
après enrichissement du potentiel, au seul choix des paramètres microscopiques :
il rend l'approximation gaussienne elle-même suspecte, sans démontrer qu'elle
est l'unique source de l'écart. Ce constat motive le diagnostic post-SCHA de la
section suivante.
...
\begin{enumerate}
  ...
  dynamique moléculaire, mais ne suffisent pas à ramollir la réponse obtenue
  par notre SCHA ; ce contraste oriente vers une sous-estimation liée à la
...
qu'elle ne resomme pas, puis à vérifier sa taille. Le but n'est pas encore de
produire une nouvelle courbe prédictive, mais de déterminer si la fermeture
gaussienne constitue effectivement une limite plausible.
...
contrôlés. Appliquer directement un flot de renormalisation au noyau réduit
n'éliminerait ni une erreur de paramétrisation microscopique ni un mode optique
absent.
...
La VCA possède d'abord une justification chimique simple : Sr et Ba appartiennent
à la même colonne du tableau périodique, celle des alcalino-terreux. Leur
substitution sur le site $A$ de la pérovskite est donc isovalente (valence
nominale $+2$) et conserve la nature chimique moyenne du sous-réseau. Cette
proximité justifie qu'une interpolation lisse des paramètres microscopiques
constitue un premier modèle de référence. La VCA, en revanche, ne décrit ni
les fluctuations locales de composition ni le désordre qu'elles induisent.
\begin{figure}[H]
\centering
...
maximal inférieur à $0.62\,\%$. Cette valeur mesure uniquement la qualité d'une
compression à deux paramètres : les cibles nominales sont des reconstructions
contraintes par la littérature et non des mesures brutes directes.
...
section~\ref{ann:scha_reponse}~\cite{TadanoTsuneyuki2015}. Elle ne démontre
